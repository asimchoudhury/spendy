...
- [ ] **B1. Roll back optimistic mutations on real server errors** — `src/hooks/useExpenses.ts` add/update/delete/import; `useCategories.ts` `importCategories`. On a non-network rejection (RLS, constraint) handlers only `setError` and never revert, so the UI shows phantom rows that aren't in the DB. Capture prior state and restore on the error branch (network branch should still enqueue, not roll back).
- [ ] **B2. Enqueue (or surface) when the fetch *throws*, not just on resolved `error`** — `useExpenses.ts:129,170,189`. The `.then(({error}))` pattern misses promise rejections (`TypeError: Failed to fetch`), the most common offline mode, so offline-created expenses are silently lost. Convert to `await` + try/catch like the bulk handlers, routing through `isNetworkError → enqueue`.
